/**
 * React Bits ShapeGrid — reference implementation for the Hero canvas grid.
 * Runtime integration uses hero-shapegrid.js (vanilla) on the static homepage.
 */
...
const ShapeGrid = ({
  ...
  useEffect(() => {
    ...
    const drawGrid = () => {
      ...
      if (isHex) {
        ...
        const offsetX = staticGrid ? 0 : ((gridOffset.current.x % squareSize) + squareSize) % squareSize;
        const offsetY = staticGrid ? 0 : ((gridOffset.current.y % squareSize) + squareSize) % squareSize;
        const cols = Math.ceil(canvas.width / squareSize) + 3;
        const rows = Math.ceil(canvas.height / squareSize) + 3;
    ...
    const updateCellOpacities = () => {
      ...
      if (staticGrid && hoverRadius > 0 && mouseRef.current.inside) {
        const { x, y } = mouseRef.current;
        const startCol = Math.floor((x - hoverRadius) / squareSize) - 1;
        const endCol = Math.ceil((x + hoverRadius) / squareSize) + 1;
        const startRow = Math.floor((y - hoverRadius) / squareSize) - 1;
        const endRow = Math.ceil((y + hoverRadius) / squareSize) + 1;

        for (let col = startCol; col <= endCol; col++) {
          for (let row = startRow; row <= endRow; row++) {
            const cellCx = col * squareSize + squareSize / 2;
            const cellCy = row * squareSize + squareSize / 2;
            const dist = Math.hypot(x - cellCx, y - cellCy);
            if (dist > hoverRadius) continue;
            const falloff = 1 - dist / hoverRadius;
            const eased = falloff * falloff * (3 - 2 * falloff);
            targets.set(`${col},${row}`, eased * 0.95);
          }
        }
      } else {
      ...
      for (const [key, opacity] of cellOpacities.current) {
        ...
        const lerp = staticGrid ? fadeSpeed : 0.15;
        const next = opacity + (target - opacity) * lerp;
    ...
    const handleMouseMove = event => {
      ...
      if (staticGrid && hoverRadius > 0) {
        mouseRef.current = { x: mouseX, y: mouseY, inside: true };
        return;
      }
  ...
  }, [direction, speed, borderColor, hoverFillColor, squareSize, shape, hoverTrailAmount, staticGrid, hoverRadius]);
